...
<div class="d-flex justify-content-between align-items-center flex-wrap">
    <div class="d-flex flex-wrp py-2 mr-3">
        ...
        $dot_section=false;
        if($paginator->lastPage()>($middle_links+4)){
            $dot_section=true;
        }
        @endphp
        ...
        @for ($i = 1; $i <= $paginator->lastPage(); $i++)
            @if ($i<=2) 
                <a href="{{ ($paginator->currentPage() == $i) ? 'javascript:void(0)' : $paginator->url($i)}}" class="btn btn-icon btn-sm border-0 btn-hover-success mr-2 my-1 {{ ($paginator->currentPage() == $i) ? ' active' : '' }}">{{ $i }}</a>
            @elseif ($i+2>$paginator->lastPage())

                @if (($paginator->currentPage()+2+$links_centers_both_side)<$paginator->lastPage())

                    @if ($right_dot_count<1 && $dot_section==true)
                    
                        @php
                        $right_dot_count++;
                        @endphp

                        <div class="btn-icon btn-sm border-0 btn-hover-success mr-2 my-1 disabled">...</div>
            ...
            @else
                ...
                @if (($paginator->currentPage()-(2+$links_centers_both_side))>1)
                    ...
                    @if ($left_dot_count<1 && $dot_section==true)
                ...
                @if ($middle_row_count<$middle_links)
                
                    @if (($paginator->currentPage()+2+$links_centers_both_side)-$paginator->lastPage()>=0)
