<ol class="dd-list">
    @foreach ($menus[$menu_id] as $menu)
    <li class="dd-item" data-id="{{$menu->id}}">
        <div class="nestable-toolbar d-flex">
            <span class="switch switch-sm switch-success mr-2">
                <label>
                    <input type="checkbox" @if ($menu->is_active==1) checked="checked"
                    @endif name="is_active[{{$menu->id}}]">
                    <span></span>
                </label>
            </span>
        ...
        @include('admin.admin-menus.single-menu',['menus'=>$menus,'menu_id'=>$menu->id])
        @endif
    </li>
    @endforeach
</ol>
