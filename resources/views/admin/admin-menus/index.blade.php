            ...
            <div class="card card-custom gutter-b">
                ...
                <div class="card-body pt-0 pb-3">
                    ...
                    <div class="col-md-8 mx-auto my-20">
                        ...
                        @if (isset($info->menus[0]))
                            <div class="mb-5" id="nestable-menu">
                                ...
                                <button type="button" class="btn btn-primary float-right"
                                    data-action="collapse-all">Collapse
                                    All</button>
                            </div>
                            ...
                            <div class="dd" id="nestable">
                                <ol class="dd-list">
                                    ...
                                    @foreach ($info->menus[0] as $menu)
                                        <li class="dd-item" data-id="{{ $menu->id }}">
                                            <div class="nestable-toolbar d-flex">
                                                <span class="switch switch-sm switch-success mr-2">
                                                    <label>
                                                        <input type="checkbox" @if ($menu->is_active == 1) checked="checked" @endif
                                                            name="is_active[{{ $menu->id }}]">
                                                        <span></span>
                                                    </label>
                                                </span>

                                                <a href="{{ route('admin.admin-menus.edit', $menu->id) }}"
                                                    class="btn btn-sm btn-icon btn-bg-warning btn-icon-white  mr-2">
                                            ...
                                            <div
                                                class="dd-handle font-weight-bold d-flex align-items-center bg-light-primary rounded p-5 ">
                                                <!--begin::Icon-->
                                            ...
                                            @if (isset($info->menus[$menu->id]))
                                                @include('admin.admin-menus.single-menu',['menus'=>$info->menus,'menu_id'=>$menu->id])
                                            @endif
                                        </li>
                                    @endforeach
                                </ol>
                        ...
                        @else
                            <h2 class="card-title align-items-start flex-column text-center">
                                <span class="card-label font-weight-bolder text-dark"> No Menus Added</span>
                            </h2>
                        @endif
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var updateOutput = function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.save-nested-admin-menus') }}",
                    type: 'GET',
                    data: {
                        'nested_menus_array': list.nestable('serialize')
                    },
