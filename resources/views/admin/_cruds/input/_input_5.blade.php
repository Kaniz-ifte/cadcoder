...
<div class="{{ isset($input->div_class) ? $input->div_class : 'col-md-3' }}">
    ...
    <span class="switch switch-outline switch-icon switch-success @error($input->name) is-invalid @enderror">
        <label>
            ...
            <input type="checkbox" value="1" @if (isset($input->update)) {{ $data->{$input->name} == '1' ? 'checked' : '' }} @else {{ isset($input->checked) ? 'checked' : '' }} @endif name="{{ $input->name }}" {{ isset($input->disabled) ? 'disabled' : '' }}>
            <span></span>
        </label>
    </span>
