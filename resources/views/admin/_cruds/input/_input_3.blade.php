...
<div class="{{ isset($input->div_class) ? $input->div_class : 'col-md-3' }}">
    <div class="image-input image-input-outline @error($input->name) is-invalid @enderror" id="{{ $input->id }}">
        <div class="image-input-wrapper"
            style="width:{{ isset($input->sample_image_width) ? $input->sample_image_width : '' }};height:{{ isset($input->sample_image_height) ? $input->sample_image_height : '' }};background-image: url({{ isset($input->update) ? Storage::url($data->{$input->name}) : asset(isset($input->sample_image) ? $input->sample_image : '') }})">
        </div>
