...
class Menu
{
    ...
    /**
     * renderDynamicMenu is a static function of
     * Menu::class generate dynamic menu
     *
     * @method \App\Classes\Theme\Menu::class renderDynamicMenu($parent_id)
     * @param  integer $parent_id
     * @return void
     */
    public static function renderDynamicMenu($parent_id = 0)
    {
        $menus = AdminMenu::where("parent_id", $parent_id)->orderBy('order', 'ASC')->get();
        foreach ($menus as $menu) {
            $is_exists = AdminMenu::where("parent_id", $menu->id)->exists();
            echo '
            <li class="menu-item' . ($is_exists ? ' menu-item-submenu' : '') . '" aria-haspopup="true" ' . ($is_exists ? 'data-menu-toggle="hover"' : '') . '>
                <a href="'
                . ($is_exists ? 'javascript:;' : route($menu->url ?? 'admin.dashboard')) . '" class="menu-link menu-toggle">'
                . ($menu->parent_id != 0 && empty($menu->icon) ?
                    ' <i class="menu-bullet menu-bullet-' . ($menu->bullet ?? 'dot') . '">
                            <span></span>
                        </i>' : '')
                . (!empty($menu->icon) ? '
                ...
                . ($is_exists ? '<i class="menu-arrow"></i>' : '') . '
                </a>
            ';
            if ($is_exists) {
                echo '
            ...
            Menu::renderDynamicMenu($menu->id);
            if ($is_exists) {
                echo '</ul> </div>';
            }
            echo '</li>';
