...
class CrudController extends Controller
{
    ...
    public function create($id)
    {
        ...
        $form_inputs = DB::table('crud_inputs')
            ->join('crud_columns', 'crud_columns.id', 'crud_inputs.crud_column_id')
            ->where('is_addable', 1)
            ->where('crud_inputs.is_active', 1)
            ->where('crud_inputs.crud_id', $id)
            ->select('crud_column_id', 'crud_inputs.is_active')->get()->groupBy('crud_column_id');
        $inputs = '[
              ';
        ...
        // $table_columns = DB::table('crud_view_columns')->join('crud_view_designs', 'crud_view_columns.id', 'crud_view_designs.crud_view_column_id')->where('crud_view_columns.is_active', 1)->where('crud_view_columns.crud_id', $id)->select('crud_view_column_id')->get()->groupBy('crud_view_column_id');
        $table_columns = [];
        $columns = '[
              ';
        ...
        $model_file_text = "";
        // $model_columns = DB::table('crud_columns')->where('crud_id', $id)->pluck('name');
